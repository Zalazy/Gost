...
update_sh() {
  ol_version=$(curl -L -s --connect-timeout 5 https://ghfast.top/https://raw.githubusercontent.com/Zalazy/Gost/refs/heads/main/gost.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [ -n "$ol_version" ]; then
    if [[ "$shell_version" != "$ol_version" ]]; then
      ...
      [yY][eE][sS] | [yY])
        wget -N --no-check-certificate https://ghfast.top/https://raw.githubusercontent.com/Zalazy/Gost/refs/heads/main/gost.sh
        echo -e "更新完成"
        exit 0
        ;;
      *) ;;
